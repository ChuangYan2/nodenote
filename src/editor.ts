/*---------------------------------------------------------------------------------------------
 *  Idea Inbox custom editor: a webview-backed editor for markdown files. The document text is the
...
export class IdeaInboxEditorProvider implements vscode.CustomTextEditorProvider {
	public static readonly viewType = 'ideaInbox.editor';
	...
	async resolveCustomTextEditor(
		...
		const pushToWebview = () => webview.postMessage({ type: 'load', title, view: parseView(document.getText()) });
		...
		const messageSub = webview.onDidReceiveMessage(async (msg: { type: string; view?: View }) => {
			if (msg.type === 'ready') {
				pushToWebview();
				return;
			}
